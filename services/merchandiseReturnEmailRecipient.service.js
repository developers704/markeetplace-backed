...
const MerchandiseReturnEmailRecipient = require('../models/merchandiseReturnEmailRecipient.model');
...
async function upsertSystemRecipient({ warehouseId, role, userId, userModel, isAdmin = false }) {
  ...
  const existing = await MerchandiseReturnEmailRecipient.findOne(filter).lean();
  if (existing) return existing;

  return MerchandiseReturnEmailRecipient.create({
    warehouseId,
    role,
    userId: userId || null,
...
async function syncWarehouseEmailRecipients(warehouseId) {
  ...
  return MerchandiseReturnEmailRecipient.find({ warehouseId: whId })
...
function formatRecipientRow(rec, warehouse) {
  ...
  if (rec.role === 'REQUESTER') {
    ...
    email = 'User who submits merchandise return';
...
async function getActiveRecipientsForOrder(warehouseId) {
  ...
  const [warehouse, recipients] = await Promise.all([
    Warehouse.findById(warehouseId)
      .select('name storeEmail districtManager corporateManager')
      .lean(),
    MerchandiseReturnEmailRecipient.find({ warehouseId, isActive: true }).lean(),
  ]);

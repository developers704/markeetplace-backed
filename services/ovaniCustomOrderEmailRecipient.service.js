...
const OvaniCustomOrderEmailRecipient = require('../models/ovaniCustomOrderEmailRecipient.model');
...
async function upsertSystemRecipient({ warehouseId, role, userId, userModel, isAdmin = false }) {
  const filter = { warehouseId, role, userId: userId || null };
  const existing = await OvaniCustomOrderEmailRecipient.findOne(filter).lean();
  if (existing) {
    const updates = {};
    ...
    if (Object.keys(updates).length) {
      await OvaniCustomOrderEmailRecipient.updateOne({ _id: existing._id }, { $set: updates });
    }
    return existing;
  }
  return OvaniCustomOrderEmailRecipient.create({
    warehouseId,
    role,
    userId: userId || null,
...
async function syncWarehouseEmailRecipients(warehouseId) {
  ...
  await upsertSystemRecipient({ warehouseId: whId, role: 'REQUESTER', userId: null });
  ...
  if (String(warehouse.storeEmail || '').trim()) {
    await upsertSystemRecipient({ warehouseId: whId, role: 'STORE_EMAIL', userId: null });
  }
  return OvaniCustomOrderEmailRecipient.find({ warehouseId: whId })
...
function formatRecipientRow(rec, warehouse) {
  ...
  if (rec.role === 'REQUESTER') {
    ...
    email = 'User who submits the inquiry';
...
async function getActiveRecipientsForOrder(warehouseId) {
  ...
  const [warehouse, recipients] = await Promise.all([
    Warehouse.findById(warehouseId)
      .select('name storeEmail districtManager corporateManager')
      .lean(),
    OvaniCustomOrderEmailRecipient.find({ warehouseId, isActive: true }).lean(),
  ]);
  ...
  const enriched = recipients.map((r) => {
    ...
    return { ...r, userEmail: user?.email || null, userName: user?.username || null };
  });

...
async function upsertSystemRecipient({ warehouseId, role, userId, userModel, isAdmin = false }) {
  ...
  const existing = await RapnetOrderEmailRecipient.findOne(filter).lean();
  if (existing) {
    const updates = {};
    ...
    if (Object.keys(updates).length) {
      await RapnetOrderEmailRecipient.updateOne({ _id: existing._id }, { $set: updates });
    }
    return existing;
  }
  return RapnetOrderEmailRecipient.create({
    warehouseId,
    role,
    userId: userId || null,
...
async function syncWarehouseEmailRecipients(warehouseId) {
  ...
  return RapnetOrderEmailRecipient.find({ warehouseId: whId })
...
async function getActiveRecipientsForOrder(warehouseId) {
  ...
  const [warehouse, recipients] = await Promise.all([
    Warehouse.findById(warehouseId)
      .select('name storeEmail districtManager corporateManager')
      .lean(),
    RapnetOrderEmailRecipient.find({ warehouseId, isActive: true }).lean(),
  ]);
